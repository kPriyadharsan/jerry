...
import { Mic, Square, Play, Pause, Loader2, RotateCcw, Volume2, Music2, ChevronDown } from 'lucide-react';
import { englishService, voiceService } from '../services';
      ...
      {Array.from({ length: barCount }).map((_, i) => {
        ...
        return (
          <div
            key={i}
            style={{
              width: 3, height: isPlaying ? undefined : baseH,
              minHeight: 4, maxHeight: 44,
              background: 'var(--accent)', borderRadius: 99,
              animation: isPlaying ? `vpBeat 0.9s ease-in-out ${delay}s infinite alternate` : 'none',
              opacity: isPlaying ? 1 : 0.35, transition: 'opacity 0.4s',
            }}
          />
        );
      })}
...
function MusicPlayer({ audioUrl, onPlayChange }) {
  ...
  useEffect(() => { setIsPlaying(false); setCurrentTime(0); setDuration(0); }, [audioUrl]);
  ...
  useEffect(() => {
    ...
    const onMeta  = () => { const d = isFinite(audio.duration) ? audio.duration : 0; setDuration(d); };
  ...
  const togglePlay = async () => {
    const audio = audioRef.current;
    if (!audio) return;
    if (isPlaying) { audio.pause(); setIsPlaying(false); if (onPlayChange) onPlayChange(false); }
    else { try { await audio.play(); setIsPlaying(true); if (onPlayChange) onPlayChange(true); } catch (e) { console.error(e); } }
  };
    ...
    <div className="w-full rounded-2xl overflow-hidden" style={{ background: 'var(--text-primary)' }}>
      ...
      <div className="px-6 pb-3 relative group">
        <div className="relative h-1.5 cursor-pointer" onClick={(e) => {
          ...
          <div className="absolute left-0 top-0 h-full rounded-full transition-all duration-75" style={{ width: `${progress}%`, background: 'var(--accent)' }} />
          <div className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-white shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" style={{ left: `calc(${progress}% - 6px)` }} />
          <input type="range" min={0} max={duration || 100} step={0.01} value={currentTime} onInput={handleSeek} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" aria-label="Seek audio" />
        </div>
      </div>
      <div className="flex items-center justify-between px-6 pb-5">
        <div className="flex items-center gap-2 w-28">
          <Volume2 size={14} className="text-white/40 shrink-0" />
          <input type="range" min={0} max={1} step={0.01} value={volume} onInput={(e) => { const v = Number(e.target.value); setVolume(v); if (audioRef.current) audioRef.current.volume = v; }} className="w-full accent-[var(--accent)] cursor-pointer h-1" aria-label="Volume" />
        </div>
        <button onClick={togglePlay} className="w-12 h-12 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-110 active:scale-95" style={{ background: 'var(--accent)' }} aria-label={isPlaying ? 'Pause' : 'Play'}>
          {isPlaying ? <Pause size={20} fill="white" className="text-white" /> : <Play size={20} fill="white" className="text-white ml-0.5" />}
        </button>
        <button onClick={() => { if (audioRef.current) { audioRef.current.currentTime = 0; setCurrentTime(0); } }} className="w-8 h-8 flex items-center justify-center text-white/40 hover:text-white/80 transition-colors" aria-label="Restart">
          <RotateCcw size={15} />
        </button>
      </div>
    </div>
...
export default function VoicePracticePage() {
  ...
  const [isLoadingTopic, setIsLoadingTopic] = useState(!location.state?.selectedTopic);

  const [timer, setTimer]                   = useState(0);
  const [results, setResults]               = useState(null);      // original score results
  const [voiceAnalysis, setVoiceAnalysis]   = useState(null);      // grammar + topics
  ...
  const {
    recorderState, setRecorderState, startRecording, stopRecording,
    pauseRecording, resumeRecording, isPaused, resetState, analyserRef,
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        try {
          ...
          const [scoreData, voiceData] = await Promise.allSettled([
            englishService.analyzeAudio(base64, cleanMime, timer),
            voiceService.analyzeVoice(base64, cleanMime, timer, selectedTopic?.name || 'General Topic', selectedTopic?.id || ''),
          ]);

          if (scoreData.status === 'fulfilled') {
            setResults(scoreData.value);
            addSession(scoreData.value);
          } else {
            console.error('Score analysis failed:', scoreData.reason);
            setResults({
              overall: 0, fluency: 0, clarity: 0, vocabulary: 0, grammar: 0,
              feedback: 'Score analysis failed. Grammar analysis may still be available.',
              strength: 'Analysis Error', improve: 'Check connection',
            });
          }

          if (voiceData.status === 'fulfilled') {
            setVoiceAnalysis(voiceData.value);
          } else {
            console.error('Voice analysis failed:', voiceData.reason);
          }
  ...
  }, [addSession, setRecorderState, timer, fetchDashboardData, selectedTopic]);
  ...
  // Sync topic if navigation state changes, else fetch next recommended
  useEffect(() => {
    if (location.state?.selectedTopic) {
      ...
      const fetchTopic = async () => {
        ...
        try {
          const data = await voiceService.getNextTopic();
          setSelectedTopic(data.nextTopic || { id: 'introducing-yourself', name: 'Introducing Yourself', category: 'Basics' });
        } catch (e) {
          console.error("Failed to fetch next topic:", e);
          setSelectedTopic({ id: 'introducing-yourself', name: 'Introducing Yourself', category: 'Basics' });
        } finally {
          setIsLoadingTopic(false);
        }
      };
  ...
  const handleReset = () => {
    ...
    // Re-fetch next topic in case the newly logged session changed recommendations
    if (!location.state?.selectedTopic) {
      setIsLoadingTopic(true);
      voiceService.getNextTopic().then(data => {
        setSelectedTopic(data.nextTopic || { id: 'introducing-yourself', name: 'Introducing Yourself', category: 'Basics' });
        setIsLoadingTopic(false);
      }).catch(e => {
        console.error("Failed to fetch next topic on reset:", e);
        setIsLoadingTopic(false);
      });
    }
  };
  ...
  const handleRetry = async () => {
    ...
    reader.onloadend = async () => {
      const base64 = reader.result.split(',')[1];
      try {
        const data = await englishService.analyzeAudio(base64, (lastMimeType || 'audio/webm').split(';')[0]);
        setResults(data);
        addSession(data);
        setRecorderState('results');
      } catch {
  ...
  return (
    <>
      ...
      <div className="w-full h-full overflow-y-auto" style={{ background: 'var(--bg)', color: 'var(--text-primary)' }}>
        <div className="max-w-[820px] mx-auto px-5 md:px-10 py-10 min-h-full flex flex-col gap-8">
          ...
          <header className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2.5 py-1.5 px-4 rounded-full text-[13px] font-medium" style={{ background: 'var(--bg-secondary)' }}>
              <span className="w-2 h-2 rounded-full animate-pulse" style={{ background: 'var(--accent)', boxShadow: '0 0 8px rgba(0,200,83,0.5)' }} />
          ...
          {recorderState !== 'results' && (
            <div className="flex flex-col items-center justify-center gap-6 py-4 animate-in fade-in duration-500">

              {/* Highlighted Recommended Topic */}
              {isLoadingTopic ? (
                <div className="w-full max-w-[380px] flex justify-center py-2">
                  <Loader2 size={24} className="animate-spin text-[var(--accent)]" />
                </div>
              ) : selectedTopic && (
                <div
                  className="w-full max-w-[380px] p-4 rounded-[14px] flex items-center justify-between animate-in fade-in cursor-default"
                  style={{ background: 'var(--bg-secondary)', border: '1.5px solid var(--accent)', boxShadow: '0 0 15px rgba(0,200,83,0.1)' }}
                >
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: 'var(--accent)' }}>Recommended Topic</p>
                    <p className="text-[15px] font-semibold" style={{ color: 'var(--text-primary)' }}>{selectedTopic.name}</p>
                  </div>
                  <span className="text-[10px] font-semibold px-2 py-1 rounded-full bg-[var(--bg)] text-[var(--text-muted)]">
                ...
                <div className="z-10 flex items-center gap-4">
                  {recorderState === 'idle' && (
                    <button
                      ...
                      style={{ background: 'var(--text-primary)', color: 'var(--bg)', boxShadow: '0 0 0 0px rgba(0,200,83,0.12)' }}
                  ...
                  {recorderState === 'processing' && (
                    <div className="relative flex items-center justify-center flex-col gap-3">
                      <div className="relative">
                        ...
                        <Loader2 size={42} className="animate-spin absolute top-0 left-0" style={{ color: 'var(--accent)', animationDirection: 'reverse', animationDuration: '0.6s' }} />
                      </div>
                      <p className="text-[12px] font-semibold animate-pulse" style={{ color: 'var(--text-muted)' }}>
              ...
              <div className="text-center min-h-[52px]">
                ...
                {recorderState === 'idle' && selectedTopic && (
                  <div className="flex flex-col items-center gap-1.5 mt-2">
                    <p className="text-[12px]" style={{ color: 'var(--text-muted)' }}>
                      Ready to practice <span className="font-semibold" style={{ color: 'var(--text-primary)' }}>"{selectedTopic.name}"</span>?
                    </p>
                  </div>
                )}
                {recorderState === 'recording' && (
                ...
                {recorderState === 'processing' && (
                  <p className="text-[14px]" style={{ color: 'var(--text-secondary)' }}>Analysing your speech…</p>
                )}
              </div>
            </div>
          )}
          ...
          {recorderState === 'results' && (
            <div className="flex-1 flex flex-col gap-6 animate-in slide-in-from-bottom-6 fade-in duration-500 pb-10">
              ...
              {results && (
                <div className="w-full rounded-[20px] p-6 relative overflow-hidden shadow-lg" style={{ background: 'var(--text-primary)', color: 'white' }}>
                  <div className="absolute top-0 right-0 w-48 h-48 rounded-full pointer-events-none" style={{ background: 'var(--accent)', opacity: 0.12, filter: 'blur(60px)', transform: 'translate(25%,-40%)' }} />
                  <div className="relative z-10 flex flex-col md:flex-row md:items-start gap-6">
                    <div className="shrink-0">
                      ...
                      <div className="flex items-end gap-3">
                        ...
                        {deltas && (
                          <span className="font-mono text-[16px] pb-2" style={{ color: deltas.overall >= 0 ? 'var(--accent)' : 'var(--danger)' }}>
              ...
              {results && deltas && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  ...
                  <div className="md:col-span-1 rounded-[18px] p-5 flex flex-col justify-between border-l-[3px] h-full" style={{ background: 'var(--bg-secondary)', borderLeftColor: 'var(--accent)' }}>
                    <div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <span className="text-[11px] font-semibold px-3 py-1 rounded-full border" style={{ background: 'var(--accent-soft)', borderColor: 'var(--accent-medium)', color: 'var(--accent)' }}>
                        ↑ {results.strength}
                      </span>
              ...
              {voiceAnalysis && (
                <VoiceAnalysisPanel
                  topic={selectedTopic?.name || 'General Topic'}
                  duration={timer}
                  score={voiceAnalysis.grammarScore}
                  mistakes={voiceAnalysis.mistakes}
                  nextTopics={voiceAnalysis.nextTopics}
              ...
              {/* Progress Trend */}
              <div className="w-full rounded-[18px] p-5 relative overflow-hidden" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', height: 140 }}>
                ...
                <div className="absolute bottom-5 left-5 right-5 top-12 flex items-end justify-between">
                  {sessions.length > 1 ? (
                    <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none" viewBox="0 0 100 40">
                      ...
                      />
                      <circle
                        cx="100" cy={40 - (sessions[sessions.length - 1].overall / 100) * 35}
                        r="2.5" fill="var(--accent)" stroke="white" strokeWidth="1"
                      />
                    </svg>
                  ) : (
              ...
              {/* CTA buttons (only shown if no voice analysis panel, else panel has its own) */}
              {!voiceAnalysis && (
